...
2. 根據當前工作區判斷 project family（Core / PA / POS / ESP）；若已知 leaf project，如 PA-UI、POS-API，先回推到對應 family。
3. 讀取 `P:\MEMORY\projects/{family}/{family}.md`。
4. 讀取 `P:\MEMORY\projects/{family}/status.md`（含 Related Lessons）。
5. 若已鎖定子專案，再讀取 `P:\MEMORY\projects/{family}/{leaf}.md`。
6. 回報工作狀態（Current Focus / Next Actions / Blocked）與相關教訓。
...
4. **三個月清理**：檢查 `journal/log.md` 中超過 90 天的條目：
   - 有延續價值 → 蒸餾後刪除
   - 無延續價值 → 直接刪除
5. 清理 session memory（`/memories/session/`）中不再需要的暫存筆記。
6. 向使用者摘要本次成果與下次待辦。
7. 顯示 `Memory has updated!`。
