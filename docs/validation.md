...
Native Windows 執行、真實 Claude／Codex 對話的自動選用與 compact／clear／idle 全生命週期未執行。Windows 路徑與舊 hash 的單元案例不能代替原生平台驗證。description 提供選用線索，不能保證每次模型都選對。没有全域安裝或修改使用者既有宿主信任設定。
