...
Before delegating, if this machine has a session-metrics collector (`{SESSION_METRICS_CMD}`), run it and capture this session's own tokens/cost/duration as a lightweight, single-session snapshot per `{MEMORY_VAULT}/handovers/handovers.md`'s "Session Metrics 段落規則"; on a machine without one, skip and record the snapshot as "not available (no collector on this machine)" — record honestly (including `fallback`/workspace-mismatch/known-zero-duration cases as "not usable, reason X"), never fabricate or aggregate across sessions, and never let this substitute for `/save`'s durable metrics.
