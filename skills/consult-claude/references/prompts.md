...
```
You are acting as an external consultant to Codex.
...
Codex 先完成自己的初步分析；可選擇不附上 Codex 的結論，以免錨定 Claude。
...
Codex currently proposes the following design:
...
Here is Codex's current position:
...
第二輪（選用，最後一次）：附上 Codex 對每項反駁的回應，要求 Claude 指出哪些反駁仍成立、哪些已被化解，以及剩餘分歧。
...
Only modify the allowed files. Do not run Codex or other agents.
