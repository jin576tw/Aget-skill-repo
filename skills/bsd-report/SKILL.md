...
若呼叫端（如 `/adp-test-verify`）在對話中提供「測試情境卡」，一律以卡片為輸入，逐項對照，**不重新改寫卡片文字**：

- `驗證目標` → `修改項目：`
- `操作步驟` → `操作步驟：`（**原文照抄**，不改寫、不合併、不重新編號）
- `截圖點` → `畫面截圖`，依卡片順序嵌入
- `驗收判定` → 併入操作步驟最後一條；「未驗證」的項目仍須產出，並在該項目註明未驗證原因（例如「（未驗證：SIT 無符合資料）」）

卡片內容已在產出階段套過「只用畫面可見元素」規則，重寫會讓技術用語回流，故禁止重新改寫。
