...
2. **裁決浮上對話**：Checker 的裁決與 rubric 分數必須輸出在對話中（如 `AC-03 PASS | R:18/20`），evaluator 只能讀對話輸出，寫在磁碟的報告它看不到。
...
4. **狀態寫磁碟**：每輪結束 append 一行至工作目錄的 `goal-evidence.md`：
   ```
   | R{輪次} | {AC/任務} | {signal} | {rubric 或 －} | {下一步或 blocked 原因} |
   ```
   防止長迴圈 context compaction 後失憶。不產生 per-worker handoff 檔（token 成本高且 evaluator 讀不到）。
